...
AI 研发平台是一个以 AI 驱动软件研发全流程的内部平台，覆盖需求分析、架构设计、代码研发、测试、发布部署，在关键节点由人审批。平台通过接入主流 Coding Agent（Codex/CodeBuddy 等）执行研发任务，沉淀领域知识让 AI 越用越准。
...
极其重要：当遇到文件引用（例如 @docs/rules/general.md）时，请使用 Read 工具按需加载。这些文件与当前的具体任务密切相关。
Instructions:
...
- 关于领域事件组件规范：@docs/rules/backend-domain-event.md
- 关于后端异常处理规范：@docs/rules/backend-exception-handling.md
