...
- 领域事件组件自身 ID 策略遵循 [@docs/rules/backend-domain-event.template.md](./backend-domain-event.template.md)，业务表内部 ID 策略遵循本文档。
...
- 数据库变更脚本仍需遵循 [@docs/rules/backend-database-change-governance.template.md](./backend-database-change-governance.template.md)。
