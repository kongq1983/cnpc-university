...
> 这是目标项目的候选规则模板，不是本指南仓库的生效规范。复制到目标项目后，必须根据实际情况评审、改写和裁剪。
...
- 涉及数据库变更发布、回滚与脚本编排，统一遵循 [@docs/rules/backend-database-change-governance.template.md](./backend-database-change-governance.template.md)。
...
- **内部 ID 统一治理**：系统内部主键类型、生成策略与外部 ID 边界统一遵循 [@docs/rules/backend-id-generation.template.md](./backend-id-generation.template.md)。
- **SQL 归位 XML**：自定义查询/更新 SQL 必须写在 Mapper XML 中，禁止在 Mapper 接口（含注解 SQL 与 default 方法）内直接编写 SQL 语句。
- **方言差异受控**：运行时 SQL 禁止在业务层散落数据库方言分支，选型边界遵循 [@docs/rules/backend-database-multi-db-compatibility.template.md](./backend-database-multi-db-compatibility.template.md)。
...
- [ ] 新增/改造表名是否为单数。
- [ ] PO 命名是否与表语义一致。
- [ ] 系统内部主键是否遵循 [@docs/rules/backend-id-generation.template.md](./backend-id-generation.template.md)，未新增 `INT`、`AUTO_INCREMENT`、`IDENTITY` 内部主键。
- [ ] 是否未新增数据库 `CHECK` 约束。
