...
- 领域对象创建过程中的内部 ID 生成必须遵循 [@docs/rules/backend-id-generation.template.md](./backend-id-generation.template.md)，领域层只能依赖 `IdGenerator` 接口，不得直接依赖具体生成器实现。
...
- 系统内部 ID 的类型、生成方式与 API 表达必须遵循 [@docs/rules/backend-id-generation.template.md](./backend-id-generation.template.md)。
...
- 创建方法必须通过参数接收 `IdGenerator` 并在方法内生成聚合根自身 ID，ID 生成细则遵循 [@docs/rules/backend-id-generation.template.md](./backend-id-generation.template.md)。
